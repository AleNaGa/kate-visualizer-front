...
export default function DeploymentsForm({ data }) {
  const [replicaChanges, setReplicaChanges] = useState({});
  const [toastMessage, setToastMessage] = useState(null); // Estado para el toast
  const { scaleDownDeployment } = useDeploymentActions();
  const { scaleUpDeployment } = useDeploymentActions();
          ...
          {data?.deployments?.length > 0 ? (
            <div className="space-y-4">
              ...
              flex items-center justify-center hover:brightness-110 hover:scale-105 transition-all ease-in-out duration-300"
                    ...
                    <button
                      onClick={() => handleStop(deployment.name, deployment.namespace)}
